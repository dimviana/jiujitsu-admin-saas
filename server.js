...
if (isProduction && cluster.isPrimary) {
    ...
    // FIX: Increase payload limit to 50mb to handle Base64 images and certificates uploads
    app.use(express.json({ limit: '50mb' }));
    app.use(express.urlencoded({ limit: '50mb', extended: true }));

    // Serve static files from the React build with aggressive caching
    ...
    // This allows lazy loading of images instead of sending huge Base64 strings in JSON
    app.get('/api/images/:type/:id', async (req, res) => {
        ...
        switch(type) {
            ...
            case 'settings': table = 'theme_settings'; break; // for logo
            case 'event': table = 'events'; break;
            default: return res.status(400).send('Invalid type');
        }

        try {
            // For settings, ID is usually 1, but we use the passed ID just in case
    ...
    app.post('/api/login', async (req, res) => {
        ...
        try {
            // Optimize: Select only needed columns for auth
            ...
            if (students.length > 0 && (students[0].password === password || !students[0].password)) {
                ...
                // Convert imageUrl to API URL if it exists
    ...
    app.get('/api/initial-data', async (req, res) => {
        try {
            ...
            const [
                ...
                pool.query('SELECT id, name, email, role, academyId, studentId, birthDate FROM users'), // Exclude imageUrl from users, assume linked to student
            ...
            const parsedStudents = students.map(s => ({ 
                ...
                documents: s.hasDocuments ? [] : [], // Documents must be fetched on demand now
    ...
    app.post('/api/students/auto-promote-stripes', async (req, res) => {
        ...
        try {
            ...
            // Optimize: limit attendance fetch for auto-promote
