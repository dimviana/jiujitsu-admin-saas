...
app.post('/api/payments/credit-card', async (req, res) => {
    ...
    try {
        ...
        const [settings] = await conn.query('SELECT mercadoPagoAccessToken, mercadoPagoPublicKey FROM theme_settings LIMIT 1');
        ...
        // NOTE: In a PCI compliant environment, you should send a TOKEN generated by frontend.
        // For this implementation, we are sending card data server-to-server as requested/implied by the constraints.
        const paymentMethodId = getPaymentMethodId(cardData.number);
        ...
        const paymentPayload = {
            ...
            // Direct card data (Caution: Only for specific approved scopes/testing)
            card: {
        ...
        await conn.query(
            ...
            [`pay_mp_${mpData.id}`, studentId, new Date(), amount]
        );
    ...
    try {
        if (academyId) {
            ...
            await pool.query(`UPDATE theme_settings SET 
                ...
                creditCardEnabled=?, efiEnabled=?, efiPixKey=?, efiPixCert=?
                WHERE id = 1`,
                [s.systemName, s.logoUrl, s.primaryColor, s.secondaryColor, s.backgroundColor, 
                 ...
                 s.creditCardEnabled, s.efiEnabled, s.efiPixKey, s.efiPixCert]
            );
        }
        res.json({ success: true });
